...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("parCountChange should return 0 for money < 0 and moneyThreshold") {
    def check(money: Int, coins: List[Int]) =
  ...
  test("parCountChange should return 1 when money == 0 and moneyThreshold") {
    def check(coins: List[Int]) =
  ...
  test("parCountChange should return 0 for money > 0 and coins = List() and moneyThreshold") {
    def check(money: Int) =
  ...
  test("parCountChange should work when there is only one coin and moneyThreshold") {
    def check(money: Int, coins: List[Int], expected: Int) =
  ...
  test("parCountChange should work for multi-coins and moneyThreshold") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, moneyThreshold(money)) == expected,
        s"parCountChange($money, $coins) should be $expected")
  ...
  test("parCountChange should work for multi-coins and totalCoinsThreshold") {
    def check(money: Int, coins: List[Int], expected: Int) =
  ...
  test("parCountChange should work for multi-coins and combinedThreshold") {
    def check(money: Int, coins: List[Int], expected: Int) =
