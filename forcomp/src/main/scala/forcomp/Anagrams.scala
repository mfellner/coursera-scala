...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = w
    .toLowerCase
    .filter(_.isLetter)
    .foldLeft(Map[Char, Int]().withDefaultValue(0))((m, c) => m.updated(c, m(c) + 1))
    .toList
    .sorted
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary
    ...
    .foldLeft(Map[Occurrences, List[Word]]().withDefaultValue(List())) {
      case (map, (occurences, word)) => map.updated(occurences, word :: map(occurences))
    }
