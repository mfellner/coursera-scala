...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.foldLeft(Map[Char, Int]().withDefault(_ => 0))((m, c) => m + (c -> (m(c) + 1))).toList
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case base if base.length < 2 => base
  ...
  def until(isSingle: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = trees match {
    case single if isSingle(trees) => single.head
    case multiple => until(isSingle, merge)(merge(multiple))
  }
